    ...
    try {
      ...
      return interaction.reply({
        embeds: [
          new MessageEmbed()
            .setColor('BLURPLE')
            .setDescription('**Số dư của bạn không đủ để chơi `Cl` :coin: :coin: :coin:\n \n Vui lòng sử dụng lệnh `Work` hay nhận tiền hàng ngày để chơi.**'),
        ],
      });
    }
...
async function updateWallet(userID, guildID, amount) {
  ...
  if (!updatedProfile) {
    throw new Error('**Số dư của bạn không đủ để chơi `Cl`. Vui lòng sử dụng lệnh `Work` hay nhận tiền hàng ngày để chơi.**');
  }
}
