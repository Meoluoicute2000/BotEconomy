...
module.exports = {
  ...
  confirm: async (interaction, embed) => {
    const msg = await interaction.reply({
      ...
      components: [
        new MessageActionRow().addComponents(
          new MessageButton().setCustomId('Tiếp tục').setStyle('SUCCESS').setLabel('Tiếp tục'),
          new MessageButton().setCustomId('Hủy bot').setStyle('DANGER').setLabel('Hủy bỏ')
        )
      ],
      fetchReply: true
    });
    ...
    if (!i)
      return {
        proceed: false,
        reason: 'Lý do: Hết thời gian hoạt động',
        i
      };

    if (i.customId === 'Tiếp tục')
      return {
        proceed: true,
        i
      };
